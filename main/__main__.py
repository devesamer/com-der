...
async def settingscallback(event):
    compress_button = Button.inline("⚙️ Compression Settings", data="main_settings")
    upload_options = Button.inline("📤 Upload Options", data="upload_options")
    back = Button.inline("Back", data="back_main")
    text = "**⚙️ Main Settings**"
    await event.edit(text, buttons=[[compress_button], [upload_options], [back]])


@bot.on(events.CallbackQuery(data="main_settings"))
async def main_settings_callback(event):
    speed_button = Button.inline("⚡️ Speed", data="compress")
    crf_button = Button.inline("📊 CRF", data="crf")
    fps_button = Button.inline("🎬 FPS", data="fps")
    back = Button.inline("Back", data="settings")
    text = "**⚙️ Compression Settings**"
    await event.edit(text, buttons=[[speed_button, crf_button, fps_button], [back]])


@bot.on(events.CallbackQuery(data="upload_options"))
async def upload_options_callback(event):
    as_video_button = Button.inline("📹 As Video", data="set_upload_video")
    as_document_button = Button.inline("📄 As Document", data="set_upload_document")
    original_thumb_button = Button.inline("🖼️ Original Thumbnail", data="original_thumb")
    change_thumb_button = Button.inline("📸 Change Thumbnail", data="change_thumb")
    back = Button.inline("Back", data="settings")
    text = "**📤 Upload Options**"
    await event.edit(text, buttons=[[as_video_button, as_document_button], [original_thumb_button, change_thumb_button], [back]])


@bot.on(events.CallbackQuery(data="set_upload_video"))
async def set_upload_video_callback(event):
    await db.set_upload_mode(doc=False)
    await event.answer("✅ Upload mode set to Video.")


@bot.on(events.CallbackQuery(data="set_upload_document"))
async def set_upload_document_callback(event):
    await db.set_upload_mode(doc=True)
    await event.answer("✅ Upload mode set to Document.")


@bot.on(events.CallbackQuery(data="change_thumb"))
...
async def compresscallback(event):
    ultrafast = Button.inline("Ultra Fast", data="ultrafast")
    veryfast = Button.inline("Very Fast", data="veryfast")
    faster = Button.inline("Faster", data="faster")
    fast = Button.inline("Fast", data="fast")
    medium = Button.inline("Medium", data="medium")
    slow = Button.inline("Slow", data="slow")
    back = Button.inline("Back", data="main_settings")
...
async def backcompresscallback(event):
    compress_button = Button.inline("⚡️ Speed", data="compress")
    crf_button = Button.inline("📊 CRF", data="crf")
    fps_button = Button.inline("🎬 FPS", data="fps")
    back = Button.inline("Back", data="settings")
    text = "**⚙️ Compression Settings**"
...
async def crfcallback(event):
    ...
    buttons = [Button.inline(str(val), data=f"set_global_crf:{val}") for val in crf_values]
    back = Button.inline("Back", data="main_settings")
...
async def fpscallback(event):
    ...
    buttons = [Button.inline(str(val), data=f"set_global_fps:{val}") for val in fps_values]
    back = Button.inline("Back", data="main_settings")
    text = "**Select Frames Per Second (FPS)**"
    await event.edit(text, buttons=[buttons[:3], buttons[3:], [back]])


@bot.on(events.CallbackQuery(pattern=r"set_global_crf:(\d+)"))
...
@bot.on(events.CallbackQuery(pattern=r"set_global_fps:(.+?)"))
